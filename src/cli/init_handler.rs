...
async fn run_complete_init(snaprag: &SnapRag) -> Result<()> {
    ...
    let db_url = std::env::var("DATABASE_URL")
        .unwrap_or_else(|_| {
            ...
            std::fs::read_to_string("config.toml")
                ...
                .unwrap_or_else(|| "postgresql://snaprag:hackinthebox_24601@192.168.1.192/snaprag".to_string())
        });
    ...
    tracing::debug!("Executing SQL via psql: {}@{}:{}/{}", user, host, port, database);
    ...
    let output = std::process::Command::new("psql")
        ...
        .arg("-U").arg(user)
        .arg("-d").arg(database)
        .arg("-f").arg(temp_sql_path)
        .arg("-v").arg("ON_ERROR_STOP=0")  // Continue on errors
        .output()?;
    ...
    if !output.status.success() {
        ...
        return Err(crate::SnapRagError::Custom(format!(
            "psql failed with exit code {:?}: {}", output.status.code(), stderr
        )));
    }
