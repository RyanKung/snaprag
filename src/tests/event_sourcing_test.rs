...
mod tests {
    ...
    fn generate_message_hash(field_name: &str, fid: i64, timestamp: i64, value: &str) -> Vec<u8> {
        ...
        format!("test_{}_{}", field_name, hash_value).as_bytes().to_vec()
    }
    ...
    async fn test_event_sourcing_insert_and_query() {
        ...
        let db = Database::from_config(&config).await.expect("Failed to connect to database");
        ...
        assert_eq!(profile.as_ref().unwrap().username, Some("alice".to_string()));
        ...
        assert!(profile.is_some());
        assert_eq!(profile.as_ref().unwrap().username, Some("alice2".to_string()));
