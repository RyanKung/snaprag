...
async fn setup_grpc_client() -> (AppConfig, HubServiceClient) {
    ...
    let client = HubServiceClient::new(grpc_endpoint).await
...
fn create_shard_chunks_request(shard_id: u32, start_block: u64, stop_block: Option<u64>) -> ShardChunksRequest {
...
async fn make_grpc_call(client: &mut HubServiceClient, request: ShardChunksRequest) -> ShardChunksResponse {
    println!("Making gRPC GetShardChunks request for shard {}, blocks {}-{}", 
             request.shard_id, 
             request.start_block_number, 
             request.stop_block_number.unwrap_or(request.start_block_number));
...
fn verify_chunk_basic_structure(chunk: &ShardChunk) {
    // Verify header
    let header = chunk.header.as_ref().expect("Chunk header should not be None");
    let height = header.height.as_ref().expect("Header height should not be None");
    ...
    // Verify commits
    let commits = chunk.commits.as_ref().expect("Chunk commits should not be None");
    let commit_height = commits.height.as_ref().expect("Commit height should not be None");
    assert!(commit_height.shard_index >= 0, "Commit shard index should be >= 0");
    assert!(commit_height.block_number >= 0, "Commit block number should be >= 0");
    assert!(!commits.signatures.is_empty(), "Commit signatures should not be empty");
}
...
fn verify_transactions(chunk: &ShardChunk, allow_fid_zero: bool) {
    for (tx_idx, transaction) in chunk.transactions.iter().enumerate() {
        if allow_fid_zero {
            ...
            assert!(transaction.fid > 0, "Transaction FID should be > 0, but found FID=0 at transaction index {}", tx_idx);
        }
        
        // Verify user messages (if any)
        for message in &transaction.user_messages {
            let message_data = message.data.as_ref().expect("Message data should not be None");
...
fn print_detailed_block_analysis(chunk: &ShardChunk) {
    ...
    for (tx_idx, transaction) in chunk.transactions.iter().enumerate() {
        ...
        *user_message_counts.entry(user_msg_count).or_insert(0) += 1;
        ...
        if tx_idx < 10 {
            println!("\n  Transaction {}:", tx_idx);
            println!("    FID: {}", fid);
            println!("    Account Root: {}", hex::encode(&transaction.account_root));
    ...
    if commits.signatures.len() > 3 {
        println!("    ... and {} more signatures", commits.signatures.len() - 3);
    }
}
...
fn print_detailed_block_data(chunk: &ShardChunk, target_block: u64) {
    ...
    if block_number == target_block {
        ...
        println!("\n📝 ALL TRANSACTIONS ({} total):", chunk.transactions.len());
        for (tx_idx, transaction) in chunk.transactions.iter().enumerate() {
...
async fn test_parse_shard_chunks_response_real_blocks_0_to_7() {
    ...
    assert!(chunk_count > 0, "Expected to receive at least one shard chunk, but got 0");
    ...
    for (i, chunk) in response.shard_chunks.iter().enumerate() {
        ...
        for (tx_idx, transaction) in chunk.transactions.iter().enumerate() {
            ...
            } else if tx_idx < 5 { // Only print first 5 transactions without messages
                println!("  Transaction {}: FID {}, {} user messages", 
                         tx_idx, 
                         transaction.fid,
                         user_message_count);
        ...
        let block_number = chunk.header.as_ref().unwrap().height.as_ref().unwrap().block_number;
        println!("  📊 Block {} Summary: {} transactions, {} with user messages, {} total user messages", 
                 block_number,
                 chunk.transactions.len(),
    ...
    println!("✅ Successfully parsed {} real shard chunks from gRPC service (blocks 0-7)", chunk_count);
}
...
async fn test_parse_shard_chunks_response_real_block_9_with_fid_zero() {
    ...
    assert!(block_9_found, "Expected to find Block 9 in the response, but it was not found");

    println!("✅ Successfully parsed {} real shard chunks from gRPC service (block 9 with FID=0)", chunk_count);
}
